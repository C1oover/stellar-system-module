...
Hooks.once("init", async () => {
	...
	game.modules.get(MODULE_ID).api = {
		...
		openEditor: (scene = canvas?.scene) =>
			SolarSystemEditor.openForScene(scene),
		applyPreset: async (scene = canvas?.scene, preset = "single") => {
			const data =
				preset === "binary"
					? buildBinaryPreset(scene)
					: buildSinglePreset(scene);
...
Hooks.on("getSceneControlButtons", (controls) => {
	...
	controls[MODULE_ID] = {
		...
		order:
			Math.max(
				0,
				...Object.values(controls).map((c) => Number(c?.order ?? 0)),
			) + 1,
		activeTool: "openEditor",
		tools: {
			openEditor: {
				name: "openEditor",
				title: "Open Editor",
			...
			autopopulateSingle: {
				name: "autopopulateSingle",
				title: "Autopopulate Single Star",
				icon: "fa-solid fa-sun",
				button: true,
				visible: true,
				order: 1,
				onChange: async (_event, active) => {
					if (active === false || !canvas?.scene) return;
					await canvas.scene.setFlag(
						MODULE_ID,
						FLAG_KEY,
						buildSinglePreset(canvas.scene),
					);
					SolarSystemEditor.openForScene(canvas.scene);
				},
			},
			autopopulateBinary: {
				name: "autopopulateBinary",
				title: "Autopopulate Binary Stars",
				...
				onChange: async (_event, active) => {
					...
					await canvas.scene.setFlag(
						MODULE_ID,
						FLAG_KEY,
						buildBinaryPreset(canvas.scene),
					);
					SolarSystemEditor.openForScene(canvas.scene);
				},
			},
...
function ensureSolarSystem(scene) {
	...
	if (!scene.getFlag(MODULE_ID, FLAG_KEY))
		scene.setFlag(MODULE_ID, FLAG_KEY, data);
	return data;
}

function normalizeSystemData(data) {
	const merged = foundry.utils.mergeObject(
		foundry.utils.deepClone(DEFAULT_SYSTEM),
		data,
		{ inplace: false, insertKeys: true, insertValues: true, overwrite: true },
	);
	merged.bodies = Array.isArray(merged.bodies)
		? merged.bodies.map(normalizeBodyData)
		: [];
	return merged;
}
...
function normalizeBodyData(body) {
	...
	return foundry.utils.mergeObject(defaults, body ?? {}, {
		inplace: false,
		overwrite: true,
		insertKeys: true,
		insertValues: true,
...
class SolarSystemEditor extends Application {
	...
	getData() {
		...
		if (!this.selectedBodyId && system.bodies.length)
			this.selectedBodyId = system.bodies[0].id;
		const selectedBody =
			system.bodies.find((b) => b.id === this.selectedBodyId) ?? null;
		const binaryGroups = Array.from(
			new Set(
				system.bodies
					.filter((b) => b.type === "star" && b.binaryGroup)
					.map((b) => b.binaryGroup),
			),
		);
		...
		return {
			...
			bodyTypeOptions: ["star", "planet", "moon"].map((value) => ({
				value,
				label: value[0].toUpperCase() + value.slice(1),
				selected: selectedBody?.type === value,
			})),
			orbitModeOptions: [
				...
				...option,
				selected: selectedBody?.orbitMode === option.value,
			})),
			parentOptions: [
				{ value: "", label: "None", selected: !selectedBody?.parentId },
				...system.bodies
					.filter((b) => b.id !== selectedBody?.id && b.type !== "moon")
					.map((b) => ({
						value: b.id,
						label: `${b.name} (${b.type})`,
						selected: selectedBody?.parentId === b.id,
					})),
			],
			targetOptions: [
				...
				...system.bodies
					...
					.map((b) => ({
						value: b.id,
						label: `${b.name} (${b.type})`,
						selected: selectedBody?.orbitTarget === b.id,
					})),
				...binaryGroups.map((group) => ({
					value: `binary:${group}`,
					label: `Binary barycenter: ${group}`,
					selected: selectedBody?.orbitTarget === `binary:${group}`,
				})),
			],
		};
	}
	...
	activateListeners(html) {
		...
		html
			.find("[data-action='autopopulate-single']")
			.on("click", () => this._applyPreset("single"));
		html
			.find("[data-action='autopopulate-binary']")
			.on("click", () => this._applyPreset("binary"));
		html
			.find("[data-action='enable-system']")
			.on("change", (event) =>
				this._updateSystemFlag(event.currentTarget.checked),
			);
		...
		html
			.find("[data-action='add-star']")
			.on("click", () => this._addBody("star"));
		html
			.find("[data-action='add-planet']")
			.on("click", () => this._addBody("planet"));
		html
			.find("[data-action='add-moon']")
			.on("click", () => this._addBody("moon"));
		html
			.find("[data-action='delete-body']")
			.on("click", () => this._deleteSelectedBody());
	...
	async _applyPreset(preset) {
		const data =
			preset === "binary"
				? buildBinaryPreset(this.scene)
				: buildSinglePreset(this.scene);
		if (preset === "single") {
	...
	async _saveForm(form) {
		...
		if (expanded.system) {
			...
			current.animationSpeed = numberOr(
				current.animationSpeed,
				expanded.system.animationSpeed,
			);
			current.backgroundColor =
				expanded.system.backgroundColor || current.backgroundColor;
			current.showLabels = !!expanded.system.showLabels;
			current.showOrbits = !!expanded.system.showOrbits;
			current.starfieldDensity = numberOr(
				current.starfieldDensity,
				expanded.system.starfieldDensity,
			);
		}
		...
		if (expanded.body && this.selectedBodyId) {
			...
			if (body) {
				...
				body.orbitRadiusX = numberOr(
					body.orbitRadiusX,
					expanded.body.orbitRadiusX,
				);
				body.orbitRadiusY = numberOr(
					body.orbitRadiusY,
					expanded.body.orbitRadiusY,
				);
				body.orbitPeriod = numberOr(
					body.orbitPeriod,
					expanded.body.orbitPeriod,
				);
				body.orbitPhase = numberOr(body.orbitPhase, expanded.body.orbitPhase);
				body.pairId = expanded.body.pairId ?? body.pairId;
				body.pairDistance = numberOr(
					body.pairDistance,
					expanded.body.pairDistance,
				);
				...
				body.binarySeparation = numberOr(
					body.binarySeparation,
					expanded.body.binarySeparation,
				);
				body.binaryPeriod = numberOr(
					body.binaryPeriod,
					expanded.body.binaryPeriod,
				);
				body.binaryPhase = numberOr(
					body.binaryPhase,
					expanded.body.binaryPhase,
				);
		...
		const body = normalizeBodyData({
			...
			orbitTarget:
				type === "star"
					? null
					: (selected?.id ??
						current.bodies.find((b) => b.type === "star")?.id ??
						null),
	...
	async _deleteSelectedBody() {
		...
		const current = getSolarSystem(this.scene);
		current.bodies = current.bodies.filter(
			(b) =>
				b.id !== this.selectedBodyId &&
				b.parentId !== this.selectedBodyId &&
				b.orbitTarget !== this.selectedBodyId,
		);
...
class SolarSystemOverlay {
	...
	_currentView() {
		...
		const focusedBody =
			this.data.bodies.find((b) => b.id === focusedId) ?? null;
		return {
			type: focusedBody ? focusedBody.type : "system",
			focusedBody,
			title: focusedBody?.name ?? this.scene.name,
			bodies: focusedBody
				? this.data.bodies.filter((b) => b.parentId === focusedBody.id)
				: this.data.bodies.filter((b) => !b.parentId),
		};
	}
	...
	_drawOrbits(view) {
		...
		for (const body of view.bodies) {
			...
			const anchor = view.focusedBody
				? this._sceneCenter()
				: this._resolveAnchor(body);
			if (!anchor) continue;
			if (body.orbitMode === "figure8") {
				this._drawFigure8(
					anchor,
					Math.max(30, body.orbitRadiusX),
					Math.max(20, body.orbitRadiusY),
				);
			} else if (body.orbitMode === "ellipse") {
				this.orbits.drawEllipse(
					anchor.x,
					anchor.y,
					Math.max(10, body.orbitRadiusX),
					Math.max(10, body.orbitRadiusY),
				);
			}
		}
	}
	...
	_drawBodies(view) {
		...
		if (view.focusedBody) {
			const center = this._sceneCenter();
			this.positionCache.set(view.focusedBody.id, {
				x: center.x,
				y: center.y,
				angle: 0,
				anchorAngle: 0,
		...
		for (const body of view.bodies) {
			const pos = view.focusedBody
				? this._resolveChildInFocusedView(body)
				: this._resolveBodyPosition(body);
	...
	_upsertBodySprite(body, position, focused = false, scaleMultiplier = 1) {
		...
		const selectedByEditor =
			SolarSystemEditor.instance?.selectedBodyId === body.id;
		...
		sprite.bodyGraphic.lineStyle(
			selectedByEditor ? 3 : 1.5,
			selectedByEditor ? 0xffffff : 0x000000,
			selectedByEditor ? 0.9 : 0.5,
		);
		...
		sprite.labelText.visible = !!(
			this.data.showLabels && body.showLabel !== false
		);
	}
	...
	_resolveBodyPosition(body) {
		...
		const angle =
			this.elapsed / Math.max(0.001, body.orbitPeriod || 10) +
			(body.orbitPhase || 0);
		let x = anchor.x;
		let y = anchor.y;
	...
	_resolveChildInFocusedView(body) {
		...
		if (body.orbitMode === "figure8") {
			...
			y +=
				(body.orbitRadiusY * 2.2 * Math.sin(angle) * Math.cos(angle)) / denom;
		} else {
	...
	_resolveStarPosition(body) {
		const center = this._sceneCenter();
		if (!body.binaryGroup)
			return { x: center.x, y: center.y, angle: 0, anchorAngle: 0 };
		const group = this.data.bodies.filter(
			(b) => b.type === "star" && b.binaryGroup === body.binaryGroup,
		);
		if (group.length < 2)
			return { x: center.x, y: center.y, angle: 0, anchorAngle: 0 };
		const angle =
			this.elapsed / Math.max(0.001, body.binaryPeriod || 10) +
			(body.binaryPhase || 0);
	...
	_resolveAnchor(body) {
		...
		if (String(body.orbitTarget).startsWith("binary:")) {
			...
			const stars = this.data.bodies.filter(
				(b) => b.type === "star" && b.binaryGroup === group,
			);
	...
	_applyPairOffset(body, point, angle) {
		if (!body.pairId)
			return { x: point.x, y: point.y, angle, anchorAngle: angle };
		const pairBodies = this.data.bodies
			.filter((b) => b.pairId && b.pairId === body.pairId)
			.sort((a, b) => a.id.localeCompare(b.id));
		const index = Math.max(
			0,
			pairBodies.findIndex((b) => b.id === body.id),
		);
		const pairAngle =
			this.elapsed / Math.max(0.001, body.pairPeriod || 5) +
			(body.pairPhase || 0) +
			index * Math.PI;
		return {
	...
	_showInfo(body) {
		...
		if (body.description)
			content.push(`<p>${escapeHtml(body.description)}</p>`);
		if (body.journalUuid)
			content.push(
				`<p><strong>Journal UUID:</strong> <code>${body.journalUuid}</code></p>`,
			);
		content.push("</div>");
		new Dialog({
			title: body.name,
	...
	_syncHud(view) {
		...
		const trailParts = [
			this.scene.name,
			...this.viewStack
				.map((id) => this.data.bodies.find((b) => b.id === id)?.name)
				.filter(Boolean),
		];
		...
		const hasInfo =
			!!view.focusedBody &&
			(!!view.focusedBody.description || !!view.focusedBody.journalUuid);
...
function makeGlowFilters(strength = 1) {
	...
	if (Noise) {
		try {
			filters.push(
				new Noise({
					noise: Math.min(0.08, 0.03 * strength),
					seed: strength * 10,
				}),
			);
		} catch (_error) {
